  ...
  align(personal-info-position, text(personal-info-font-size)[    
    #{
      ...
      let items = (
        ...
        contact_item(youtube-username, link-type: "https://www.youtube.com/", prefix: "youtube.com/"),
      )
...
#let period_worked_override(start-date, end-date) = {
  // sanity checks
  assert.eq(type(start-date), datetime)
  assert(type(end-date) == datetime or type(end-date) == str)
  ...
  return [
      _#start-date.display("[month repr:short] [year]")_ -- 
      #if (
        (end-date.month() == datetime.today().month()) and 
        (end-date.year() == datetime.today().year())
      ) [
        ...
        _#end-date.display("[month repr:short] [year]")_
      ]
  ]
}
...
#let education-heading(institution, location, degree, major, start-date, end-date, body) = {
  ...
  if (major != "") {
    generic_2x2(
      ...
      [#degree, #major], period_worked_override(start-date, end-date)
    )
  } else {
    generic_2x2(
      (70%, 30%),
      [*#institution*], [*#location*], 
      [#degree], period_worked_override(start-date, end-date)
    )
  }
...
#let education-heading-hs(institution, location, diploma, start-date, end-date, body) = {
  // sanity checks
  assert.eq(type(start-date), datetime)
  assert(type(end-date) == datetime or type(end-date) == str)
  ...
  generic_2x2(
    (70%, 30%),
    [*#institution*], [*#location*], 
    [#diploma], period_worked(start-date, end-date)
  )
  v(-0.2em)
  if body != [] {
    v(-0.4em)
